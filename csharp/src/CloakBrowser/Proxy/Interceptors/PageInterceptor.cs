...
namespace CloakBrowser.Proxy.Interceptors
{
    public class PageInterceptor
    {
        ...
        public async Task ClickAsync(string selector, PageClickOptions? options = null)
        {
            ...
            {
                await CloakBrowser.Human.Mouse.HumanIdleAsync(_rawMouse, (float)_pageState.X, (float)_pageState.Y, callCfg);
            }
            ...
            var target = CloakBrowser.Human.Mouse.ClickTarget(new Clip { X = (float)finalBox.X, Y = (float)finalBox.Y, Width = (float)finalBox.Width, Height = (float)finalBox.Height }, isInput, callCfg);
            if (!force)
            {
                await Actionability.CheckPointerEventsAsync(_page, selector, target.X, target.Y, timeout);
            }

            await CloakBrowser.Human.Mouse.HumanMoveAsync(_rawMouse, (float)_pageState.X, (float)_pageState.Y, target.X, target.Y, callCfg);
            _pageState.X = target.X;
            _pageState.Y = target.Y;

            await CloakBrowser.Human.Mouse.HumanClickAsync(_rawMouse, isInput, callCfg);
        }
        ...
        public async Task HoverAsync(string selector, PageHoverOptions? options = null)
        {
            ...
            var target = CloakBrowser.Human.Mouse.ClickTarget(new Clip { X = (float)finalBox.X, Y = (float)finalBox.Y, Width = (float)finalBox.Width, Height = (float)finalBox.Height }, false, callCfg);
            if (!force)
            {
                await Actionability.CheckPointerEventsAsync(_page, selector, target.X, target.Y, timeout);
            }
        ...
        public async Task TypeAsync(string selector, string text, PageTypeOptions? options = null)
        {
            ...
            await CloakBrowser.Human.Keyboard.HumanTypeAsync(_page, _rawKeyboard, text, callCfg, cdp);
        }
        ...
        public async Task FillAsync(string selector, string value, PageFillOptions? options = null)
        {
            ...
            var cdp = await EnsureCdpAsync();
            await CloakBrowser.Human.Keyboard.HumanTypeAsync(_page, _rawKeyboard, value, callCfg, cdp);
        }
        ...
        private class MouseProxyInterceptor
        {
            ...
            public async Task MoveAsync(float x, float y, MouseMoveOptions? options = null)
            {
                ...
                await CloakBrowser.Human.Mouse.HumanMoveAsync(_raw, (float)_state.X, (float)_state.Y, x, y, _state.Config);
                _state.X = x;
                _state.Y = y;
            }
            ...
            {
                await MoveAsync(x, y);
                await CloakBrowser.Human.Mouse.HumanClickAsync(_raw, false, _state.Config);
            }
        }
        ...
        private class KeyboardProxyInterceptor
        {
            ...
            public async Task TypeAsync(string text, KeyboardTypeOptions? options = null)
            {
                ...
                await CloakBrowser.Human.Keyboard.HumanTypeAsync(_page, _raw, text, _state.Config, cdp);
            }
        }
    }
}
